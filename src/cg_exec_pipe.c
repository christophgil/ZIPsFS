...
#ifndef WITH_DEBUG_THROTTLE_DOWNLOAD
...
static long cg_read_gzip(const int in, const int out, void (*progress)(void*), void *progress_para){
  if (WITH_DEBUG_THROTTLE_DOWNLOAD)log_entered_function("WITH_DEBUG_THROTTLE_DOWNLOAD active");
  errno=0;
  ...
  for (int i=0;(n=gzread(gz,buf,sizeof(buf)-1))>0; i++){
    ...
    if (WITH_DEBUG_THROTTLE_DOWNLOAD){fputc('l',stderr); usleep(1000*1000);}
  }
