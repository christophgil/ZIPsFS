  ...
  FOR(fromArgv,0,2){
	...
	for(int iLine=0; fromArgv?(iLine<annotations_n):(getline(&line,&capacity,f)!=-1); iLine++){
	  if (fromArgv){
		...
		if (l>capacity && !(line=realloc(line,(capacity=l)))) DIE("realloc"); // cppcheck-suppress memleakOnRealloc
...
#define C(K)   v=NULL; if (!strncmp(k,K,sizeof(K)-1)){ v=sizeof(K)-1+k; match=true;}
		...
		if (v){
		  ...
		  if (!r->with_timeout) warning(WARN_CONFIG,propertypath,"The line %d  '%s...' will be ignored, since the root-path does not start with triple-slash.",iLine+1,k);
		}
		C(ROOT_PROPERTY_PATH_DENY"=");  if (v) r->exclude_vp=split_paths_by_colon(v,exclude_l,r->exclude_vp);
		C(ROOT_PROPERTY_PATH_ALLOW"="); if (v) r->starts_vp= split_paths_by_colon(v,starts_l, r->starts_vp);
...
static void zpath_set_realpath(zpath_t *zpath, const char *rp1,const char *rp2){
  ...
  if (rp1) zpath_strcat(zpath,rp1);
  if (rp2) zpath_strcat(zpath,rp2);
...
#define C(f)   zpath->f=vipa->f
  ...
  const int vp_l=zpath->virtualpath_l=vipa->vp_l;//-cut;
  zpath_strncat(zpath,vipa->vp, vp_l);
...
static bool zpath_stat(const int opt_filldir_findrp, zpath_t *zpath){
  ...
  if (!ok){
	ok=r?stat_from_cache_or_direct_or_async(opt_filldir_findrp,RP(),RP_L(),&zpath->stat_rp,r): !stat(RP(),&zpath->stat_rp);
	if (ok) zpath->stat_vp=zpath->stat_rp;
	IF1(WITH_PRELOADDISK, if (!ok && path_with_gz_exists(zpath)) ok=true);
	if (ok){
...
static bool test_realpath_pfx(const bool dirFileconversion,  int opt_filldir_findrp, zpath_t *zpath, root_t *r){
 ...
 filter_ok:
  ...
  if (!append_realpath_warnings_errors(zpath)){
	zpath_strncat(zpath,r->rootpath,r->rootpath_l);
	if (dirFileconversion) zpath_strcat(zpath,DIR_FILECONVERSION);
	zpath_strncat(zpath,vp0+r->pathpfx_l,vp0_l-r->pathpfx_l);
  }
  ZPATH_COMMIT_HASH(zpath,realpath);
...
static bool zpath_expand_symlinks(zpath_t *zpath){
  ...
  if (ok){
	zpath_set_realpath(zpath,target,NULL);
...
static bool find_realpath_for_root(const int opt_filldir_findrp,zpath_t *zpath,root_t *r){
  if (r){
	if (r!=_root_writable && DIR_REQUIRES_WRITABLE_ROOT(zpath->dir)) return false;
	if (r==_root_writable && zpath->dir==DIR_EXCLUDE_FIRST_ROOT) return false;
...
static bool find_realpath_any_root( int opt_filldir_findrp,zpath_t *zpath,const root_t *onlyThisRoot){
  ...
  const long roots=(onlyThisRoot?(1<<rootindex(onlyThisRoot)):-1) & search_file_which_roots(VP(),VP_L(), ZPATH_IS_FILECONVERSION(zpath));
  ...
  if (!found IF1(WITH_FILECONVERSION,&&!ZPATH_IS_FILECONVERSION(zpath)) && !onlyThisRoot && !config_not_report_stat_error(VP(),VP_L()) IF1(WITH_INTERNET_DOWNLOAD, && !net_is_internetfile(VP(),VP_L()))){
...
static void fhandle_init(fHandle_t *d, const zpath_t *zpath){
  //log_entered_function("iiiiiiiiiiiiiiiii %p %s",d,VP());
  *d=FHANDLE_EMPTY;
...
static ino_t zpath_make_inode(const zpath_t *zpath, const int entryIdx){

  return make_inode(zpath->stat_rp.st_ino,zpath->root,entryIdx, RP());
...
#define X(lower,upper) if ((opt_filldir_findrp&(1<<COMPRESSION_##upper)) && cg_endsWith(0,name,name_l,"."#lower,sizeof(#lower))) filler_add(0,filler,buf,name,name_l-sizeof(#lower),sfx,st,no_dups);
	  ...
	  if (!config_do_not_list_file(RP(),u,u_l)){
		*dirname_from_zip=0;
		...
		if (*dirname_from_zip){
		  ...
		  filler_add(opt_filldir_findrp|(zpath->dir==DIR_PLAIN?0:zpath->root->preload_flags),filler,buf,u,u_l,isDIR?NULL:append_ext,&st,no_dups);
		}
	  }
	}
	directory_destroy(&dir);
...
static bool special_file_set_stat(struct stat *st, const virtualpath_t *vipa){
  ...
  bool ok=false;
  if (id>SFILE_BEGIN_IN_RAM || id==SFILE_INFO){
	...
	ok=true;
  }else if (id && id<SFILE_WITH_REALPATH_NUM){
...
#define B(l) if (!*buf) strncpy(buf,vipa->vp,l); buf[l]=0
  if (vipa->dir!=DIR_PLAIN){
	vipa->zipfile_l=virtual_dirpath_to_zipfile(vipa->vp, vipa->vp_l, &vipa->zipfile_cutr, &vipa->zipfile_append);
  }
...
static int xmp_unlink(const char *vpath){
  ...
  if (!found) return -ENOENT;
  IF1(WITH_INTERNET_DOWNLOAD,if (zpath->dir==DIR_INTERNET && VP_L()>DIR_INTERNET_L+6 && config_internet_must_not_delete(VP()+(DIR_INTERNET_L+1),VP_L()-(DIR_INTERNET_L+1))) return -EPERM);
  return !ZPATH_ROOT_WRITABLE()?-EPERM:  minus_val_or_errno(unlink(RP()));
}
...
static int open_for_reading(const virtualpath_t *vipa, struct fuse_file_info *fi){
  ...
  if (!fh){ /* ID for fHandle_t  */
	...
	if (!ff && find_realpath_any_root(FILLDIR_FROM_OPEN,zpath,NULL)  IF1(WITH_FILECONVERSION,&&!fileconversion_remove_if_not_uptodate(zpath))){
	  IF1(WITH_PRELOADDISK,      if (!ff && (is_preloaddisk_zpath(zpath) || vipa->dir==DIR_PRELOADED_UPDATE)) ff=FHANDLE_PREPARE_ONCE_IN_RW);
	...
	//	log_debug_now(ANSI_MAGENTA"vipa->vp: %s ff:%d path_with_gz_exists:%d",VP(),ff,path_with_gz_exists(zpath));
...
#define A(n) filler_add(0,filler,buf,n,0,NULL,NULL,&no_dups);
  ...
  FOR(i,0,SFILE_NUM) if (SFILE_NAMES[i] && vipa.dir && (vp_l==vipa.dir_l || !vp_l)  && vipa.dir==SFILE_PARENTS[i]) A(SFILE_NAMES[i]);
  IF1(WITH_CCODE, if (c_readdir(zpath,buf,filler,NULL)) ok=true);
  if (!(vipa.flags&ZP_VP_STRIP_PFX)){
	if (vipa.dir==DIR_ZIPsFS  && vipa.vp_l==DIR_ZIPsFS_L){ /* Childs of folder ZIPsFS */
#define N(dir) A((dir)+DIR_ZIPsFS_L+1)
	  N(DIR_FILECONVERSION); N(DIR_PRELOADDISK_R); N(DIR_PRELOADDISK_RC);  N(DIR_PRELOADDISK_RZ); IF1(WITH_PRELOADRAM,N(DIR_PRELOADED_UPDATE)); N(DIR_INTERNET); N(DIR_FIRST_ROOT); N(DIR_EXCLUDE_FIRST_ROOT); N(DIR_PLAIN);
#undef N
	  ok=true;
	}
...
static void fhandle_prepare_in_fuse_read_or_write(fHandle_t *d,const int open_flags){
  ...
  IF1(WITH_PRELOADDISK,       if (!Done && open_flags==O_RDONLY && is_preloaddisk_zpath(&d->zpath)){   do_open=false,d->errorno=preloaddisk(d);});
  if (do_open){
	if (!RP_L() || !zpath_stat(0,zpath) ){ // || !zpath->root
	  d->errorno=ENOENT;
...
#undef S
  ...
  { /* dot_ZIPsFS */
	...
	FOR(id,0,SFILE_WITH_REALPATH_NUM){
	  if (id!=SFILE_LOG_WARNINGS && id!=SFILE_LOG_ERRORS && id!=SFILE_INFO) continue;
	  const char *name=SFILE_NAMES[id];
...
// SFX_UPDATE  /local/filesystem/fuse-3.16.2/lib/helper.c   /local/filesystem/fuse-3.16.2/lib/fuse_loop_mt.c   fuse_session_loop_mt_312(
// DIR_INTERNET_UPDATE SFX_UPDATE  net_parse_header
// cg_httpheader_parse_line():218
// structures all /home/cgille/.ZIPsFS/DB/pdbj.ZIPsFS.properties
// rootpath _root_writable  link() linkat() cg_fork_exec  is_404
// CLEAR_CACHE_LEN CLEAR_DIRCACHE
// ENOENT zpath_set_atime IS_LINUX
// COUNT_FHANDLE_CONSTRUCT SFILE_INFO
// S_IRGRP S_IXGRP


// xz lrzip path_with_gz_exists preloaddisk_writable_realpath():22
