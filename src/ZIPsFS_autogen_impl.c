...
/// Only ZIPsFS_configuration_autogen.c  is customizable.                          ///
//////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////
static bool _aimpl_matches(const char *vp, const int vp_l,const struct _autogen_config *s){ /*NOT_TO_HEADER*/
  FOR(i,0,N_PATTERNS){
    if (s->patterns[i]){
      bool ok=false;
...
//////////////////////////////////////////////////////////////////////
/// For a given generated file find the respective configuration.  ///
//////////////////////////////////////////////////////////////////////
...
/// Return: true if there are further indicating that the function will be called again with incremented iTh_dependency ///                                                          ///
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#define with_recursion 1 // DEBUG_NOW
#define EPIPE_IF_ERRFILE 1
  if (with_recursion){
  ...
  if (s->concurrent_computations<2){
    lock(mutex_autogen);
    struct stat st_fail,st_out,st_in;
    if (PROFILED(stat)(infile,&st_in)){
      perror(infile); /* No infile */
      res=-errno;
      warning(WARN_AUTOGEN|WARN_FLAG_ERRNO,infile,"aimpl_run");
    } else if (!stat(outfile,&st_out)){
      if  (!(ok=cg_timespec_b_before_a(st_out.ST_MTIMESPEC,st_in.ST_MTIMESPEC)) && unlink(outfile)) warning(WARN_AUTOGEN|WARN_FLAG_ERRNO,outfile,"unlink");
    } else if (EPIPE_IF_ERRFILE && !stat(errfile,&st_fail)){
  ...
  //log_exited_function("outfile: %s res: %d",outfile,res);
  return res;
}
...
////////////////////////////////////////////////////////////////////
static bool aimpl_ends_like_a_generated_file(const char *virtualpath,const int virtualpath_l){
  FOREACH_AUTOGEN(is,s){
    if (cg_endsWith(virtualpath,virtualpath_l,s->ext,s->_ext_l) &&_aimpl_matches(virtualpath,virtualpath_l-s->_ext_l,s)) return true;
  }
  return false;
}
