...
static void info_counts_by_filetype(FILE *file){
  ...
  FOR(pass,0,c){
    ...
    ht_entry_t *ee=HT_ENTRIES(&_ht_count_by_ext);
    RLOOP(i,c){
...
static void log_memusage_ht(FILE *file){
  ...
  BEGIN_PRE();
  ht_report_memusage(file,NULL,IS_HTML());
#define X(ht) ht_report_memusage(file,&ht,IS_HTML());
  XMACRO_HT_GLOBAL();
#undef X
  foreach_root(r){
    F("\n%sRoot %s%s\n",IS_HTML()?"<B>":ANSI_BOLD,r->rootpath,IS_HTML()?"</B>":ANSI_RESET);
#define X(ht) ht_report_memusage(file,&r->ht,IS_HTML());
    XMACRO_HT_ROOT();
#undef X
  }
  LF();
  F("\n%sMemory storages%s\n",IS_HTML()?"<B>":ANSI_BOLD,IS_HTML()?"</B>":ANSI_RESET);
  ht_report_memusage(file,&HT_REPORT_MEMUSAGE_END,IS_HTML());
  int wroteHeader=0;
  foreach_root(r){
#define X(s) if (!r->s.capacity) continue; if (!wroteHeader++) F("\n%sRoot %s%s\n",IS_HTML()?"<B>":ANSI_BOLD,r->rootpath,IS_HTML()?"</B>":ANSI_RESET);mstore_report_memusage(file,&r->s);
    XMACRO_MSTORE_ROOT();
#undef X
  }
  LF();
    ...
    while(!feof(f)) {
      ...
      *mapname=0;
      ...
      if (k>=6){
        ...
        total+=size;
        if (!strchr(mapname,'/') && size>=THRESHOLD_MALLOC_MMAP){
          RLOOP(isHeader,(!headerDone++?2:1)){
...
<B>ZIPsFS:</B> <A href=\""HOMEPAGE"\">"HOMEPAGE"</A><BR> \n");
  ...
  if (!(flags&PRINT_INFO_SHORT)){
    ...
    P(IF01(WITH_PRELOADRAM,"!WITH_PRELOADRAM",enum_when_preloadram_zip_S[_preloadram_policy]));
    P("<BR>\n");
