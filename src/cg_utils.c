...
static void cg_print_file_mode(mode_t m,FILE *f){
  char txt[11];
  int i=0;
...
// S_IRWXU
#define  cg_set_executable(path)  cg_set_st_mode_flag(true,path,S_IEXEC)
#define  cg_set_readonly(path)    cg_set_st_mode_flag(false,path,S_IWUSR|S_IWGRP|S_IWOTH)
static bool cg_set_st_mode_flag(const bool add,const char *path, const mode_t mode){
  struct stat st;
  return path && *path && !stat(path,&st) && !chmod(path, add?(st.st_mode|mode):(st.st_mode&~mode));
}

#define cg_is_executable(path)  (cg_stat_mode(path)&S_IEXEC)
static mode_t cg_stat_mode(const char *path){
  struct stat st;
  return stat(path,&st)?0:st.st_mode;
}
...
#define TMP_FOR_FILE(tmp,f)   char tmp[f?strlen(f)+99:0]; if (f)_tmp_for_file(tmp,f)
...
static bool cg_rename_tmp_outfile(const char *tmp, const char *f){
  ...
  if (!cg_file_exists(tmp)) return false;
  cg_unlink(f);
  cg_rename(tmp,f);
