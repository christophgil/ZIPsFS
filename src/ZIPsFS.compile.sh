...
sanitize='-rdynamic -g -fsanitize=address '
...
try_compile(){
    ...
    local cmd="$CCOMPILER $cc_opts  $c $ld_opts $RPATHS $LIBFUSE $LIBZIP -o $x"
...
detect_fuse_version(){
    ...
    for p in -lfuse3 -lfuse2 -lfuse1 -lfuse ''; do
        ...
        if try_compile ${x##*/} \
                       ...
                       'printf("-DFUSE_MAJOR_V=%d -DFUSE_MINOR_V=%d",FUSE_MAJOR_VERSION,FUSE_MINOR_VERSION); if (argc>99) fuse_main(0,NULL,NULL,NULL);'  \
                       "$IPATHS" \
                       "$LPATHS $p" >/dev/null; then
            $x  && echo " $p " && return 0
...
main(){
    ...
    ! fuse_version=$(echo $(detect_fuse_version)) && echo "$ANSI_FG_RED Failed to compile tiny test program with libfuse $ANSI_RESET"  && return 1
    LL+=" ${fuse_version##* } "
    opts+=" ${fuse_version% -l*} "
    ...
    [[ -s $x ]] && echo "${ANSI_RED}Warning, $x exists $ANSI_RESET"
    {
        ...
        cat $cc |sed -n 's|^\(static .*)\) *{$|\1;|p' |grep -v -e NOT_TO_HEADER  >$g
...
EOF
    ...
    local c=${x}_compilation.sh
    {
        ...
        echo "# This file has been created with $SRC"$'\n\n'$CCOMPILER  $NOWARN "$opts" $IPATHS -O0 -D_FILE_OFFSET_BITS=64  $sanitize  $DIR/ZIPsFS.c $LIBFUSE $LIBZIP $RPATHS   $LPATHS $LL $RPATHS -o $x
    }|tee $c
    chmod +x $c
    ls -l $c
