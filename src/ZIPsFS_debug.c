...
/*     if (dir){ */
/*       const directory_core_t *d=&dir->core; */
/*       log_msg("%s():%d "ANSI_INVERSE"Directory"ANSI_RESET" rp: %s files_l: %d  destroyed: %d debug: %d\n",fn,line,DIR_RP(), d->files_l, dir->dir_is_destroyed,dir->debug); */
...
static void _viamacro_directory_print(const char *func,const int line,const directory_t *dir,const int maxNum){
  ...
  fprintf(stderr,"\n"ANSI_INVERSE"%s:%d  Directory %p '%s'   files_l: %d/%d  destroyed: %s success: %s  ht-intern: %s\n"ANSI_RESET,func,line,dir,DIR_RP(),d->files_l,  dir->files_capacity, yes_no(dir->dir_is_destroyed), yes_no(dir->dir_is_success),yes_no(NULL!=hti));
...
*/
#if WITH_DEBUG_DIRCACHE_COMPARE_CACHED
...
#define print_realpath() dde_print("dir_realpath  '%s'  '%s'\n",DIR_RP(A),DIR_RP(B))
  ...
  directory_core_t a=A->core,b=B->core;
...
#endif //WITH_DEBUG_DIRCACHE_COMPARE_CACHED




#if WITH_DEBUG_TRACK_FALSE_GETATTR_ERRORS
...
#endif //WITH_DEBUG_TRACK_FALSE_GETATTR_ERRORS
