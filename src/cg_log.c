...
static bool _logIsSilent=false;
...
static void log_already_exists(const char *format,...){
  if(!_logIsSilent){
    prints(" Already exists "ANSI_RESET);
    log_argptr();
  }
}
static void log_failed(const char *format,...){
  if(!_logIsSilent){
    log(ANSI_FG_RED" $$ %d Failed "ANSI_RESET,getpid());
    log_argptr();
  }
}
static void log_warn(const char *format,...){
  if(!_logIsSilent){
    log(ANSI_FG_RED" $$ %d Warn "ANSI_RESET,getpid());
    log_argptr();
  }
}
static void log_error(const char *format,...){
  if(!_logIsSilent){
    log(ANSI_FG_RED" $$ %d Error "ANSI_RESET,getpid());
    log_argptr();
  }
}
static void log_succes(const char *format,...){
  if(!_logIsSilent){
    prints(ANSI_FG_GREEN" Success "ANSI_RESET);
    log_argptr();
  }
}
static void log_debug_now(const char *format,...){
  if(!_logIsSilent){
    log(ANSI_FG_MAGENTA" Debug "ANSI_RESET" %lx ",pthread_self());
    log_argptr();
  }
}
static void log_entered_function(const char *format,...){
  if(!_logIsSilent){
    log(ANSI_INVERSE">>>"ANSI_RESET);
    log_argptr();
  }
}
static void log_exited_function(const char *format,...){
  if(!_logIsSilent){
    prints(ANSI_INVERSE"< < < <"ANSI_RESET);
    log_argptr();
  }
}
static void log_cache(const char *format,...){
  if(!_logIsSilent){
    log(ANSI_MAGENTA" $$ %d CACHE"ANSI_RESET" ",getpid());
    log_argptr();
  }
}
...
static void _log_mthd(char *s,int count){
  if (isPowerOfTwo(count)) log(" %s=%d ",s,count);
}
...
static void warning(const unsigned int channel,const char* path,const char *format,...){
  ...
  if (!channel){
    ...
    if (path){
      static FILE *file;
      if (!(file=fopen(path,"w"))) log_debug_now(" Error fopen  %s 'n",path), perror("");
    }
#if defined _ht_dot_c_end
    ht_init(&ht,7);
#endif
  ...
  for(int j=2;--j>=0;){
    FILE *f=j?(_logIsSilent?NULL:LOG_STREAM):(toFile?f:NULL);
    if (!f) continue;
