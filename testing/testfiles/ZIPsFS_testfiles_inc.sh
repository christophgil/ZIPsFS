...
test_pattern(){
		...
		for ((i=0; i<repeats; i++)); do
				...
				if strings $vp | grep "$pattern"; then
						...
						echo "${ANSI_FG_RED} FAILED:$i pattern:'$pattern'  $ANSI_RESET"
				fi
				if ((i)); then
						set -x
						rm $vp
...
print_src(){
		...
		local pattern="${2:-}"
		[[ -n $pattern ]] &&  test_pattern  1 $pattern        "$p@SOURCE.TXT"
}
...
while getopts 'vdfsm:' o; do
		case $o in
				...
				d) rm -r "/home/_cache/$USER/ZIPsFS/modifications";;
				v) VERBOSE=-v;;
