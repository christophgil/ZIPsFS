...
run_ZIPsFS(){
        ...
        #        $EXE  "$@" $VERBOSE -k -l 33GB  -c rule  "$MODI/" /$REMOTE1/  $FTP_DIRS  : -o $fuseopt    $MNT
                $EXE  "$@" $VERBOSE -k -l 33GB  -c rule  "$MODI/"   /$REMOTE1/   $FTP_DIRS  : -o $fuseopt    $MNT
         set +x
         nl "$HOME/.ZIPsFS/$(realpath $MNT|tr / _|sed 's|^_||1')/errors.log" | head
        set +x
...
EXE="$HOME/git_projects/ZIPsFS/ZIPsFS"
while getopts 'x' o; do
    case $o in
        x) EXE=$OPTARG;;
        *) echo "wrong option $o" >/dev/stderr;;
    esac
done
